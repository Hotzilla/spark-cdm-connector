...
class CDMIdentifier(options: CaseInsensitiveStringMap) extends Identifier{
  ...
  override def namespace(): Array[String] = Array(cdmOptions.storage, cdmOptions.container, cdmOptions.manifestFileName).filter(_ != null)

  override def name(): String = Option(cdmOptions.entity).getOrElse("")
}
